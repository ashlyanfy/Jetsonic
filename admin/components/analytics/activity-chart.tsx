...
import type { SiteEventDailyPoint } from "@/lib/types";
import type { Period } from "./period";

type Series = "visitors" | "views" | "actions";

const SERIES: Array<{ value: Series; en: string; ru: string; dot: string }> = [
  { value: "visitors", en: "Visitors", ru: "Посетители", dot: "bg-brand-700" },
  { value: "views", en: "Page views", ru: "Просмотры", dot: "bg-accent-500" },
  { value: "actions", en: "Actions", ru: "Действия", dot: "bg-violet-500" },
];

/** Daily visitor activity for the selected period. Same visual language as LeadsChart. */
export function ActivityChart({ period }: { period: Period }) {
  ...
  const query = useQuery({
    queryKey: ["events-daily", period.from, period.to],
    queryFn: () =>
      api<SiteEventDailyPoint[]>("/events/daily", { query: { from: period.from, to: period.to } }),
  ...
  const labels =
    lang === "ru"
      ? { title: "Активность по дням", subtitle: "Посетители и их действия на сайте", total: "Сумма" }
      : { title: "Daily activity", subtitle: "Visitors and their actions on the site", total: "Total" };
  ...
  const points = data.map((d, i) => {
    ...
    const y = padY + usableH - (d[series] / max) * usableH;
    return { x, y, date: d.date, value: d[series] };
  });

  // A single day renders as one centered point — the path math below needs 2+.
  const linePath =
    points.length > 1
      ? points
          .map((p, i) => {
    ...
    <section className="overflow-hidden rounded-3xl border border-[rgba(6,44,73,0.08)] bg-white/90 p-6 shadow-[0_18px_45px_rgba(6,44,73,0.08)] backdrop-blur-sm">
      ...
      <div className="relative">
          ...
          {linePath && (
            <path
              ...
              style={{
                ...
                animation: pathLength ? "draw-activity 1.4s ease-out forwards" : undefined,
              }}
            />
          )}
          ...
          {hovered && (
            <g pointerEvents="none">
              <line
                ...
                strokeDasharray="3 3"
              />
              <circle cx={hovered.x} cy={hovered.y} r={6} fill="#075b94" />
            </g>
          )}
        </svg>

        <style jsx>{`
          @keyframes draw-activity {
            to {
              stroke-dashoffset: 0;
            }
          }
        ...
        {hovered && (
          ...
          >
            <div className="text-[10px] font-bold uppercase tracking-wider text-accent-400">{hovered.date}</div>
            <div className="text-base">{hovered.value}</div>
          </div>
        )}
